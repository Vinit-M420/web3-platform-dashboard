...
export default function Capital() {
    ...
    return (
    <div className="rounded-lg border border-gray-800 p-4 flex flex-col gap-5 w-fit" >
        ...
        { CapitalPerf[capitalDuration].items.map((item, index) => (
                <div key={index} className="flex flex-col gap-2">
                    <p className="dark:text-gray-400">{item.title}</p>
                    <div className="flex justify-between items-center">
                        <span className={`${item.color} h-7 rounded`} style={{ width: `${item.percent}%` }} />
                        <p className="font-bold text-xl">{item.percent}%</p>
                    </div>    
                </div>
            ))    
        } 

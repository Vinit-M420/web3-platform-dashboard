    ...
    <div className="rounded-lg border dark:border-gray-700 border-gray-200 p-4 flex flex-col justify-between dark:text-gray-400 text-gray-500 max-w-full" >
        <div className="flex flex-col lg:flex-row sm:justify-between gap-2 lg:gap-0 mb-5">
            <div className="flex flex-col gap-2 text-white">
                ...
                <span className="flex gap-2 items-center">
                    <p className="font-bold text-xl md:text-3xl dark:text-white text-black">$10,234.23</p>
                    <span className="flex gap-1 bg-red-50 text-red-500 text-sm p-1 items-center justify-center rounded-2xl hover:bg-red-100">
                        <CircleArrowDown className="w-3 h-3"/>
                        <p className="lg:text-md text-xs">11.8%</p>
                    </span>
                </span>
                <div className="block lg:hidden mt-2">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => setCollateralDuration("month")}>This Month</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => setCollateralDuration("day")}>This Day</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => setCollateralDuration("last 3 month")}>Last 3 Months</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => setCollateralDuration("last 6 month")}>Last 6 Months</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {collateralCompositions[collateralDuration].items.map((con, index) => (  
                <div key={index} className="rounded-lg border dark:border-gray-700 border-gray-200 p-2 flex flex-col justify-around gap-2">
                    <span className="flex gap-2 items-center">
                        <span className={`${con.color} rounded-full lg:w-3 lg:h-3 w-2 h-2`} />
                        <p className="dark:text-gray-400 text-gray-500 lg:text-md md:text-sm text-xs">{con.title}</p>
                    </span>
                    ...
                    <span className="flex gap-2 ml-1 items-center">
                        <p className="font-bold text-xl lg:text-2xl text-gray-700 dark:text-gray-100">
                            <CountUp
                                ...
                                />
                        %</p>
                        <p className="text-green-500 lg:text-md md:text-sm text-xs">+{con.growth}%</p>
                    </span>
                </div>   
            ))}
        </div>
